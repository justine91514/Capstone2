...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(0, 90, 'Discount Reports', 0, false, 'C', 0, '', 0, false, 'T', 'M');
...
$pdf->Cell(70, 8, 'Discount Name', 1, 0, 'C', 1); // Adjusted width for Category Name
$pdf->Cell(70, 8, 'Value (%)', 1, 1, 'C', 1); // Adjusted width for Category Name

// Fetch data from the database
$connection = mysqli_connect("localhost", "root", "", "dbpharmacy");
$query = "SELECT * FROM discount_list"; // Modify to your actual table name
$result = mysqli_query($connection, $query);

// Data rows
$pdf->SetFillColor(255);
$pdf->SetTextColor(0);
$pdf->SetX(76); // Set X position to 40
$fill = false;

while ($row = mysqli_fetch_assoc($result)) {
    $pdf->SetFont('helvetica', '', 10); // Set font to regular (no bold or italic)
    $pdf->Cell(10, 8, $row['id'], 1, 0, 'C', $fill); // Adjust cell width
    $pdf->Cell(70, 8, $row['discount_name'], 1, 0, 'C', $fill); // Adjusted width for Category Name
    $pdf->Cell(70, 8, $row['value'], 1, 1, 'C', $fill); // Adjusted width for Category Name
    $pdf->SetX(76);
